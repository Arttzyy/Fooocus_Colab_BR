    ...
    with gr.Row():
        with gr.Column(scale=2):
            ...
            with gr.Row(elem_classes='advanced_check_row'):
                ...
                enhance_checkbox = gr.Checkbox(label='Aprimorar', value=modules.config.default_enhance_checkbox, container=False, elem_classes='min_check')
                ...
                with gr.Tabs(selected=modules.config.default_selected_image_input_tab_id):
                    with gr.Tab(label='Upscale or Variation', id='uov_tab') as uov_tab:
                        with gr.Row():
                            ...
                            with gr.Column():
                                uov_method = gr.Radio(label='Ampliar ou Variação:', choices=flags.uov_list, value=modules.config.default_uov_method)
                                gr.HTML('<a href="https://github.com/lllyasviel/Fooocus/discussions/390" target="_blank">\U0001F4D4 Documentation</a>')
                    with gr.Tab(label='Prompt de Imagem', id='ip_tab') as ip_tab:
                        with gr.Row():
                            ip_images = []
                            ip_types = []
                            ip_stops = []
                    ...
                    with gr.Tab(label='Pintar ou Expandir', id='inpaint_tab') as inpaint_tab:
                        with gr.Row():
                            with gr.Column():
                                ...
                                inpaint_additional_prompt = gr.Textbox(placeholder="Describe what you want to inpaint.", elem_id='inpaint_additional_prompt', label='Prompt Adicional para Pintura', visible=False)
                                outpaint_selections = gr.CheckboxGroup(choices=['Left', 'Right', 'Top', 'Bottom'], value=[], label='Direção de Expansão')
                            ...
                            with gr.Column(visible=modules.config.default_inpaint_advanced_masking_checkbox) as inpaint_mask_generation_col:
                                ...
                                inpaint_mask_cloth_category = gr.Dropdown(label='Categoria de Roupa',
                                                             choices=flags.inpaint_mask_cloth_category,
                                                             value=modules.config.default_inpaint_mask_cloth_category,
                                                             visible=False)
                                inpaint_mask_dino_prompt_text = gr.Textbox(label='Prompt de Detecção', value='', visible=False, info='Use singular whenever possible', placeholder='Descreva o que deseja detectar.')
                                example_inpaint_mask_dino_prompt_text = gr.Dataset(
                                    samples=modules.config.example_enhance_detection_prompts,
                                    label='Lista Rápida de Prompts de Detecção',
                                ...
                                with gr.Accordion("Advanced options", visible=False, open=False) as inpaint_mask_advanced_options:
                                    inpaint_mask_sam_model = gr.Dropdown(label='Modelo SAM', choices=flags.inpaint_mask_sam_model, value=modules.config.default_inpaint_mask_sam_model)
                                ...
                                generate_mask_button = gr.Button(value='Gerar máscara a partir da imagem')
                    ...
                    with gr.Tab(label='Descrever', id='describe_tab') as describe_tab:
                        with gr.Row():
                            with gr.Column():
                            ...
                            with gr.Column():
                                ...
                                describe_apply_styles = gr.Checkbox(label='Aplicar Estilos', value=modules.config.default_describe_apply_prompts_checkbox)
                                describe_btn = gr.Button(value='Descrever esta Imagem em Prompt')
                                describe_image_size = gr.Textbox(label='Tamanho da Imagem e Tamanho Recomendado', elem_id='describe_image_size', visible=False)
                    ...
                    with gr.Tab(label='Aprimorar', id='enhance_tab') as enhance_tab:
                        with gr.Row():
                            with gr.Column():
                                enhance_input_image = grh.Image(label='Use with Enhance, skips image generation', source='upload', type='numpy')
                                gr.HTML('<a href="https://github.com/lllyasviel/Fooocus/discussions/3281" target="_blank">\U0001F4D4 Documentation</a>')

                    with gr.Tab(label='Metadados', id='metadata_tab') as metadata_tab:
                        with gr.Column():
                            metadata_input_image = grh.Image(label='For images created by Fooocus', source='upload', type='pil')
                            metadata_json = gr.JSON(label='Metadados')
            ...
            with gr.Row(visible=modules.config.default_enhance_checkbox) as enhance_input_panel:
                with gr.Tabs():
                    with gr.Tab(label='Upscale or Variation'):
                        with gr.Row():
                            with gr.Column():
                                enhance_uov_method = gr.Radio(label='Ampliar ou Variação:', choices=flags.uov_list,
                    ...
                    for index in range(modules.config.default_enhance_tabs):
                        with gr.Tab(label=f'#{index + 1}') as enhance_tab_item:
                            enhance_enabled = gr.Checkbox(label='Ativar', value=False, elem_classes='min_check',
                                                          container=False)

                            enhance_mask_dino_prompt_text = gr.Textbox(label='Prompt de Detecção',
                                                                       info='Use singular whenever possible',
                                                                       placeholder='Descreva o que deseja detectar.',
                                                                       interactive=True,
                            ...
                            with gr.Accordion("Detection", open=False):
                                enhance_mask_model = gr.Dropdown(label='Modelo de Geração de Máscara',
                                                                 choices=flags.inpaint_mask_models,
                                                                 value=modules.config.default_enhance_inpaint_mask_model)
                                enhance_mask_cloth_category = gr.Dropdown(label='Categoria de Roupa',
                                                                          choices=flags.inpaint_mask_cloth_category,
                                                                          value=modules.config.default_inpaint_mask_cloth_category,
                                ...
                                with gr.Accordion("SAM Options",
                                    ...
                                    enhance_mask_sam_model = gr.Dropdown(label='Modelo SAM',
        ...
        with gr.Column(scale=1, visible=modules.config.default_advanced_checkbox) as advanced_column:
            with gr.Tab(label='Settings'):
                ...
                performance_selection = gr.Radio(label='Desempenho',
                                                 choices=flags.Performance.values(),
                                                 value=modules.config.default_performance,
                                                 elem_classes=['performance_selection'])

                with gr.Accordion(label='Proporções de Aspecto', open=False, elem_id='aspect_ratios_accordion') as aspect_ratios_accordion:
                    aspect_ratios_selection = gr.Radio(label='Proporções de Aspecto', show_label=False,
                ...
                image_number = gr.Slider(label='Quantidade de Imagens', minimum=1, maximum=modules.config.default_max_image_number, step=1, value=modules.config.default_image_number)

                output_format = gr.Radio(label='Formato de Saída',
                                         choices=flags.OutputFormat.list(),
                                         value=modules.config.default_output_format)

                negative_prompt = gr.Textbox(label='Prompt Negativo', show_label=True, placeholder="Type prompt here.",
                                             info='Describing what you do not want to see.', lines=2,
                                             elem_id='negative_prompt',
                                             value=modules.config.default_prompt_negative)
                seed_random = gr.Checkbox(label='Aleatório', value=True)
                image_seed = gr.Textbox(label='Semente', value=0, max_lines=1, visible=False) # workaround for https://github.com/gradio-app/gradio/issues/5354
            ...
            with gr.Tab(label='Models'):
                with gr.Group():
                    ...
                    refiner_switch = gr.Slider(label='Alternar Refinador em', minimum=0.1, maximum=1.0, step=0.0001,
                ...
                with gr.Group():
                    ...
                    for i, (enabled, filename, weight) in enumerate(modules.config.default_loras):
                        with gr.Row():
                            lora_enabled = gr.Checkbox(label='Ativar', value=enabled,
                                                       elem_classes=['lora_enable', 'min_check'], scale=1)
                            lora_model = gr.Dropdown(label=f'LoRA {i + 1}',
                                                     choices=['None'] + modules.config.lora_filenames, value=filename,
                                                     elem_classes='lora_model', scale=5)
                            lora_weight = gr.Slider(label='Peso', minimum=modules.config.default_loras_min_weight,
            ...
            with gr.Tab(label='Avançado'):
                guidance_scale = gr.Slider(label='Escala de Orientação', minimum=1.0, maximum=30.0, step=0.01,
                                           value=modules.config.default_cfg_scale,
                                           info='Higher value means style is cleaner, vivider, and more artistic.')
                sharpness = gr.Slider(label='Nitidez da Imagem', minimum=0.0, maximum=30.0, step=0.001,
                                      value=modules.config.default_sample_sharpness,
                                      info='Higher value means image and texture are sharper.')
                gr.HTML('<a href="https://github.com/lllyasviel/Fooocus/discussions/117" target="_blank">\U0001F4D4 Documentation</a>')
                dev_mode = gr.Checkbox(label='Modo de Depuração para Desenvolvedores', value=modules.config.default_developer_debug_mode_checkbox, container=False)

                with gr.Column(visible=modules.config.default_developer_debug_mode_checkbox) as dev_tools:
                    with gr.Tab(label='Debug Tools'):
                        adm_scaler_positive = gr.Slider(label='Escalador Positivo de Orientação ADM', minimum=0.1, maximum=3.0,
                                                        step=0.001, value=1.5, info='The scaler multiplied to positive ADM (use 1.0 to disable). ')
                        adm_scaler_negative = gr.Slider(label='Escalador Negativo de Orientação ADM', minimum=0.1, maximum=3.0,
                        ...
                        black_out_nsfw = gr.Checkbox(label='Ocultar NSFW', value=modules.config.default_black_out_nsfw,
                                                     interactive=not modules.config.default_black_out_nsfw,
